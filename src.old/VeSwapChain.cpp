...
VeSwapChain::~VeSwapChain() {
    ...
    vkDestroyRenderPass(device.device(), renderPass,
                        nullptr);
...
void VeSwapChain::createSwapChain() {
    ...
    // swap chain, so the implementation is allowed to
    // create a swap chain with more. That's why we'll first
    // query the final number of images with
    // vkGetSwapchainImagesKHR, then resize the container
    // and finally call it again to retrieve the handles.
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(
            ...
            vkCreateSemaphore(
                ...
                &renderFinishedSemaphores[i]) !=
                VK_SUCCESS ||
            vkCreateFence(device.device(), &fenceInfo,
    ...
    for (const auto &avaliablePresentMode :
        ...
        switch (avaliablePresentMode) {
            case VK_PRESENT_MODE_MAILBOX_KHR:
                std::cout
                    << "Avaliable Present Mode: Mailbox"
                    << std::endl;
                break;
            ...
            case VK_PRESENT_MODE_IMMEDIATE_KHR:
                std::cout
                    << "Avaliable Present Mode: Immediate"
                    << std::endl;
                break;
            ...
            case VK_PRESENT_MODE_MAX_ENUM_KHR:
                std::cout
                    << "Avaliable Present Mode: Max Enum"
                    << std::endl;
                break;
            default:
                std::cout
                    << "Avaliable Present Mode: Unkown"
                    << std::endl;
                break;
        }
    }
