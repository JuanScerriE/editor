...
VeApp::~VeApp() {
    vkDestroyPipelineLayout(veDevice.device(),
...
void VeApp::createPipelineLayout() {
    assert(sizeof(SimplePushConstantData) <=
               veDevice.properties.limits
...
void VeApp::drawFrame() {
    uint32_t imageIndex;
    auto result =
        veSwapChain->acquireNextImage(&imageIndex);
