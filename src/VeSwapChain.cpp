...
VeSwapChain::~VeSwapChain() {
  ...
  if (swapChain != nullptr) {
    vkDestroySwapchainKHR(device.device(), swapChain,
                          nullptr);
    swapChain = nullptr;
  }

  for (int i = 0; i < depthImages.size(); i++) {
    vkDestroyImageView(device.device(), depthImageViews[i],
                       nullptr);
    vkDestroyImage(device.device(), depthImages[i],
                   nullptr);
    vkFreeMemory(device.device(), depthImageMemorys[i],
                 nullptr);
  }
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkDestroySemaphore(device.device(),
                       ...
                       nullptr);
    vkDestroyFence(device.device(), inFlightFences[i],
                   nullptr);
  }
}

VkResult VeSwapChain::acquireNextImage(
    uint32_t *imageIndex) {
  ...
  VkResult result = vkAcquireNextImageKHR(
      ...
      imageAvailableSemaphores
          [currentFrame],  // must be a not signaled
                           // semaphore
...
VkResult VeSwapChain::submitCommandBuffers(
  ...
  if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
    vkWaitForFences(device.device(), 1,
                    &imagesInFlight[*imageIndex], VK_TRUE,
                    UINT64_MAX);
  }
  ...
  auto result = vkQueuePresentKHR(device.presentQueue(),
                                  &presentInfo);
...
void VeSwapChain::createSwapChain() {
  ...
  // we only specified a minimum number of images in the
  // swap chain, so the implementation is allowed to create
  // a swap chain with more. That's why we'll first query
  // the final number of images with
  // vkGetSwapchainImagesKHR, then resize the container and
...
void VeSwapChain::createImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
    ...
    if (vkCreateImageView(
            device.device(), &viewInfo, nullptr,
...
void VeSwapChain::createRenderPass() {
  ...
  if (vkCreateRenderPass(device.device(), &renderPassInfo,
                         nullptr,
...
void VeSwapChain::createDepthResources() {
  ...
  for (int i = 0; i < depthImages.size(); i++) {
    ...
    if (vkCreateImageView(device.device(), &viewInfo,
...
VkPresentModeKHR VeSwapChain::chooseSwapPresentMode(
  ...
  for (const auto &avaliablePresentMode :
       availablePresentModes) {
    switch (avaliablePresentMode) {
      case VK_PRESENT_MODE_MAILBOX_KHR:
        std::cout << "Avaliable Present Mode: Mailbox"
                  << std::endl;
        break;
      case VK_PRESENT_MODE_FIFO_KHR:
        std::cout << "Avaliable Present Mode: Fifo"
                  << std::endl;
        break;
      case VK_PRESENT_MODE_IMMEDIATE_KHR:
        std::cout << "Avaliable Present Mode: Immediate"
                  << std::endl;
        break;
      case VK_PRESENT_MODE_FIFO_RELAXED_KHR:
        std::cout << "Avaliable Present Mode: Fifo Relaxed"
                  << std::endl;
        break;
      case VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR:
        std::cout << "Avaliable Present Mode: Shared "
                     "Demand Refresh"
                  << std::endl;
        break;
      ...
      case VK_PRESENT_MODE_MAX_ENUM_KHR:
        std::cout << "Avaliable Present Mode: Max Enum"
                  << std::endl;
        break;
      default:
        std::cout << "Avaliable Present Mode: Unkown"
                  << std::endl;
        break;
    }
  }
  ...
  //     std::endl; return availablePresentMode;
  //   }
  // }

  std::cout << "Present mode: V-Sync" << std::endl;
  return VK_PRESENT_MODE_FIFO_KHR;
  // std::cout << "Present mode: Relaxed V-Sync" <<
  // std::endl; return VK_PRESENT_MODE_FIFO_RELAXED_KHR;
}

VkExtent2D VeSwapChain::chooseSwapExtent(
