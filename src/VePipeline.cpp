...
VePipeline::~VePipeline() {
    vkDestroyShaderModule(veDevice.device(),
                          vertShaderModule, nullptr);
    vkDestroyShaderModule(veDevice.device(),
                          fragShaderModule, nullptr);
    vkDestroyPipeline(veDevice.device(), graphicsPipeline,
                      nullptr);
}

std::vector<char> VePipeline::readFile(
...
void VePipeline::createGraphicsPipeline(
    ...
    const std::string& fragFilepath,
    const PipelineConfigInfo& configInfo) {
    assert(configInfo.piplineLayout != VK_NULL_HANDLE &&
           "Cannot create graphics pipeline: no "
           "pipelineLayout provided in configInfo");
    assert(configInfo.renderPass != VK_NULL_HANDLE &&
           "Cannot create graphics pipeline: no renderPass "
    ...
    if (vkCreateGraphicsPipelines(
            veDevice.device(), VK_NULL_HANDLE, 1,
...
void VePipeline::defaultPipelineConfigInfo(
    ...
    configInfo.rasterizationInfo.depthBiasConstantFactor =
        0.0f;
    configInfo.rasterizationInfo.depthBiasClamp = 0.0f;
    configInfo.rasterizationInfo.depthBiasSlopeFactor =
        0.0f;

    configInfo.multisampleInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
    configInfo.multisampleInfo.sampleShadingEnable =
        VK_FALSE;
    configInfo.multisampleInfo.rasterizationSamples =
        VK_SAMPLE_COUNT_1_BIT;
    configInfo.multisampleInfo.minSampleShading =
        1.0f;  // Optional
    configInfo.multisampleInfo.pSampleMask =
        nullptr;  // Optional
    configInfo.multisampleInfo.alphaToCoverageEnable =
        VK_FALSE;  // Optional
    ...
    configInfo.colorBlendInfo.logicOp =
        VK_LOGIC_OP_COPY;  // Optional
    configInfo.colorBlendInfo.attachmentCount = 1;
    configInfo.colorBlendInfo.pAttachments =
        &configInfo.colorBlendAttachment;
    configInfo.colorBlendInfo.blendConstants[0] =
        0.0f;  // Optional
    configInfo.colorBlendInfo.blendConstants[1] =
        0.0f;  // Optional
    configInfo.colorBlendInfo.blendConstants[2] =
        0.0f;  // Optional
    configInfo.colorBlendInfo.blendConstants[3] =
        0.0f;  // Optional
    ...
    configInfo.depthStencilInfo.depthBoundsTestEnable =
        VK_FALSE;
    configInfo.depthStencilInfo.minDepthBounds =
        0.0f;  // Optional
    configInfo.depthStencilInfo.maxDepthBounds =
        1.0f;  // Optional
    configInfo.depthStencilInfo.stencilTestEnable =
        VK_FALSE;
...
void VePipeline::createShaderModule(
    ...
    if (vkCreateShaderModule(veDevice.device(), &createInfo,
                             nullptr,
